package com.elara.music.recognition
...
internal object ShazamSignatureGenerator {
    ...
    /**
     * Hanning window: w[i] = 0.5 * (1 - cos(2Ï€*(i+1)/2049)) for i=0..2047.
     *
    ...
    private class SignatureGeneratorState {
        ...
        // Band â†’ list of peaks (bands 0..3)
        ...
        fun process(pcm: ShortArray): String {
            ...
            while (offset + 128 <= pcm.size) {
                // Match C++ stopping condition: stop when BOTH timeâ‰¥max AND peaksâ‰¥max
        ...
        private fun doFFT() {
            // Build windowed excerpt from ring buffer (oldest â†’ newest)
