package com.elara.music.utils.cipher
...
object PlayerConfigStore {
    private const val TAG = "Elara_CipherConfig"
    ...
    // Note: names must not start with "player_" â€” PlayerJsFetcher.writeToCache() purges
    ...
    // cooldown only arms in that case â€” it exists to protect the config host from repeat hits,
    ...
    internal fun writeAtomic(file: File, content: String) {
        ...
        if (!tmp.renameTo(file)) {
            // renameTo can fail on some filesystems â€” fall back to a direct write.
